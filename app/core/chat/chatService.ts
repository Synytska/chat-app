...
const BASE_URL = 'http://192.168.1.6:4000'; // Змініть на вашу IP-адресу
...
export const sendMessage = (data: {id:string, room_id: string, message: string, user: string, timestamp: { hour: number, mins: number } }): void => {
  socket.emit('newMessage', data);
};

export const subscribeToRoomMessages = (callback: (message: Message) => void) => {
